...
class CampTT extends JFrame implements ActionListener
{
  static int ano=1;
  int rcnt=0;
...
ttemp=Integer.parseInt(bid.getText());
		...
		if(ttemp < 20)
		 ps=s.con.prepareStatement("select sid,fname,lname from student where sid in (select sid from camp where c1= ?)");
		else if(ttemp < 30)
		 ps=s.con.prepareStatement("select sid,fname,lname from student where sid in (select sid from camp where c2= ?)");
		else if(ttemp < 40)

		 ps=s.con.prepareStatement("select sid,fname,lname from student where sid in (select sid from camp where c3= ?)");
		else if(ttemp < 50)
		 ps=s.con.prepareStatement("select sid,fname,lname from student where sid in (select sid from camp where c4= ?)");
		else if(ttemp < 60)
		 ps=s.con.prepareStatement("select sid,fname,lname from student where sid in (select sid from camp where c5= ?)");
		else if(ttemp < 70)
		 ps=s.con.prepareStatement("select sid,fname,lname from student where sid in (select sid from camp where c6= ?)");
