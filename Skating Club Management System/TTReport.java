...
Color c;
	public TTReport()
	{
		super ("TimeTrial Reports");
	       	JLabel background=new JLabel(new ImageIcon(ClassLoader.getSystemResource ("pics/w1.jpg")));
				  this.add(background);
		...
		b=new JButton("Batch Report");
		stud=new JButton("Student Report");
		oa=new JButton("Overall Average");
		back.setBounds(50,40,140,30);
  		b.setBounds(500,140,180,30);
		stud.setBounds(500,180,180,30);
...
//		background.add(oa);
		...
		oat.setFont(f);
		oat.setForeground(c);
		H.setBounds(100,245,500,30);
		lb.setBounds(180,140,200,30);
		ls.setBounds(180,180,200,30);
...
e.printStackTrace();
		...
		bid=new JTextField();
		sid=new JTextField();
		bid.setBounds(320,140,100,30);
		background.add(bid);
		sid.setBounds(320,180,100,30);
		background.add(sid);
	...
	try
	   {	if(e.getSource()==back)
		...
		if(e.getSource()==b || e.getActionCommand().equals("enter"))
		{
			...
			{
		i=0;
		s.sid=Integer.parseInt(bid.getText());
		...
		else if(e.getSource()==stud || e.getActionCommand().equals("enter"))
		{
